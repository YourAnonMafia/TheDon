...
For liability reasons we do need to do the shpeel of "Don't use for malicious 
purposes" and "Educational purposes only".

@author Don Gh0st <https://twitter.com/DonGh0st>
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print "{0} TheDon's strikes hit. ({1} Failed)".format(self.counter[0], self.counter[1])
...
def usage():
    print
    print color.OKAYBLUE + '###############################################################################' + color.END
    print
    print
    print                              THEDON_BANNER
    print  
...
def main():
    ...
    try:
        ...
        thedon = TheDon(url)
        thedon.useragents = useragents
        thedon.nr_workers = workers
        thedon.method = method
        thedon.nr_sockets = socks
